...
** Python
Using =eglot= for interacting with an LSP backend. The Python support requires [[https://github.com/palantir/python-language-server][python-language-server]] from PIP. For minimal stress, just install '[all]'.
#+BEGIN_SRC emacs-lisp
  (defmodule python3
