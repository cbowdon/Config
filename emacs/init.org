  ...
  (defmodule clojure
    ...
    (use-package cider
      ...
      :config (progn
                ...
                ;; don't really use these anymore, just for posterity
...
The below setup includes autocompletion with =compliment= and namespace refreshing with =tools.namespace=, both of which need setting up as aliases in =~/.clojure/deps.edn=.
  ...
  (defmodule clojure-inf
    ...
    (use-package inf-clojure
      ...
      :config (progn
                ...
                ;; Make sure we use compliment, clojure.tools.namespace, and start a socket
                ;; - these aliases are defined in my ~/.clojure/deps.edn as
                ;; {:socket {:jvm-opts ["-Dclojure.server.repl={:port,50505,:accept,clojure.core.server/repl}"]}
                ;;  :compliment {:extra-deps {compliment {:mvn/version "RELEASE"}}}
                ;;  :tools.namespace {:extra-deps {org.clojure/tools.namespace {:mvn/version "RELEASE"}}}}
                ;; Make sure the REPL is able to use compliment and tools.namespace
                (add-hook 'inf-clojure-mode-hook #'inf-clojure-load-tooling)
                (add-hook 'inf-clojure-mode-hook #'eldoc-mode)
                (define-key inf-clojure-mode-map (kbd "C-c C-t") 'inf-clojure-load-tooling)
    ...
    (defun run-clojure-socket-repl ()
      (interactive)
      (async-shell-command "clojure -A:compliment:tools.namespace:socket"))

    (global-set-key (kbd "C-x C-!") 'inf-clojure-connect)
    (global-set-key (kbd "C-x C-@") 'run-clojure-socket-repl)

    (defun inf-clojure-load-tooling ()
      (interactive)
      (message "Loading Clojure tooling (compliment, tools.namespace)")
      (inf-clojure-eval-string "(require '[compliment.core] '[clojure.tools.namespace.repl])"))
