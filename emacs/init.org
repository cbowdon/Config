...
I also include =npm-mode= to save me opening a terminal every 2 minutes.
...
#+BEGIN_SRC emacs-lisp
  (defmodule react
    ...
    (defun npm-mode-npm-run-test ()
      "Run the 'npm run test' command on a project script."
      (interactive)
      (npm-mode--exec-process "npm run test"))

    (evil-leader/set-key
      "nii" 'npm-mode-npm-install
      "nis" 'npm-mode-npm-install-save
      "nid" 'npm-mode-npm-install-save-dev
      "nr" 'npm-mode-npm-run
      "nt" 'npm-mode-npm-run-test
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar eshell-prompt-bg-color "#3d3d68"
    "Color of the prompt block background")

  (defvar eshell-prompt-fg-color "#ffffff"
    "Color of the prompt block foreground")

  (defvar eshell-prompt--separators
    (make-hash-table))

  (defun eshell-prompt--lhs (fg-color bg-color)
    (let ((hash-key (concat "curve_right_" fg-color "_" bg-color)))
      (or (gethash hash-key eshell-prompt--separators))
    (puthash hash-key (curve-right-xpm bg-color fg-color) eshell-prompt--separators)))

  (defun eshell-prompt--rhs (fg-color bg-color)
    (let ((hash-key (concat "curve_left_" fg-color "_" bg-color)))
      (or (gethash hash-key eshell-prompt--separators))
    (puthash hash-key (curve-left-xpm fg-color bg-color) eshell-prompt--separators)))
