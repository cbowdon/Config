...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key repl-map (kbd "C-s C-m") 'sql-mysql)
  (define-key repl-map (kbd "C-s C-p") 'sql-postgres)
  (define-key repl-map (kbd "C-s C-s") 'sql-sqlite)
...
** Clojure sans CIDER
CIDER is amazing, but you can get 80% of the benefits with 1% of the code, so I'm trying out this setup from [[https://martintrojer.github.io/clojure/2014/10/02/clojure-and-emacs-without-cider][Martin Trojer]]. I've made some minor changes from his blog post:

- =use-package= everywhere
- =company-etags= is now built in to =company-mode=
- I've already got =projectile= elsewhere
- my preferred REPL is simply =clojure= with the /dev/ and /test/ aliases since most projects use those
- for some reason the `lisp-eval-*` functions aren't bound in my config so I've added these again

It's not quite perfect yet - completion isn't working, or finding tags. But it doesn't seem to interfere with CIDER. Since all the bindings go into =clojure-mode-map=, CIDER will shadow the keybindings if jacked in. This makes it possible to "upgrade" to a proper CIDER experience.

#+BEGIN_SRC emacs-lisp
  (defmodule clojure-sans-cider
    "Init module for Clojure without CIDER. Even more a work in progress."
    ...
    (setq-default inferior-lisp-program "clojure -A:dev:test")
    (define-key clojure-mode-map (kbd "C-c C-i") 'run-lisp)
    (define-key clojure-mode-map (kbd "C-c C-c") 'lisp-eval-defun)
    (define-key clojure-mode-map (kbd "C-c C-e") 'lisp-eval-last-sexp)
    (define-key clojure-mode-map (kbd "C-c C-k") 'lisp-eval-buffer)
    ...
    (use-package align-cljlet
      :config (global-set-key (kbd "C-c C-a") 'align-cljlet))

    (use-package paredit
      :config (add-hook 'clojure-mode-hook 'paredit-mode))

    ;; TODO Not quite sure why completion doesn't work - disabled for now to avoid interfering with CIDER
    ;; might be that `clojure-complete' or `swank-clojure' is required in .clojure/deps.edn
    ;; 
    ;; (use-package company
    ;;   :config (progn
    ...
    ;; don't want this, at least not in global
    ;; (global-set-key (kbd "C-c C-s") 'clojure-toggle-keyword-string)

    ;; TODO this errors, should figure out why
    ;; have replaced with `lisp-eval-buffer' in the meantime
    ...
    (defun lisp-eval-buffer ()
      (interactive)
      (save-excursion ;; necessary? not sure
