...
#+BEGIN_SRC emacs-lisp
  (defmodule clojure-inf
    ...
    (defun inf-clojure-socket-repl ()
      (interactive)
      (inf-clojure-enable)
      (let* ((deps "{:deps {compliment {:mvn/version \"RELEASE\"} org.clojure/tools.namespace {:mvn/version \"RELEASE\"}}}")
             (reqs "(require '[compliment.core] '[clojure.tools.namespace.repl])")
        ...
        (message "Starting REPL: %s" cmd)
        (start-process-shell-command "Clojure socket REPL" nil cmd)))
